...
def create_app():
    ...
    if not config.get('entry_point',False):
        ...
        if not config.get('login',False):
            ...
            @app.get(prefix_url+f"{config.get('entry_point')}"+"/prueba")
            ...
            @app.post(prefix_url+f"{config.get('entry_point')}"+"/prueba")
            async def prueba(nombre: Annotated[str, Form()]):
                headers = {'Location': '/cv/mar/pruebaFinal'}
                return Response(headers=headers, status_code=statuss2.HTTP_307_TEMPORARY_REDIRECT)

            @app.post(prefix_url+f"{config.get('entry_point')}"+"/pruebaFinal")
            async def final(nombre: Annotated[str, Form()], edad: int = Form(5)):
